...
classdef SDSP_v1_0_BDGram
    ...
    methods
        ...
        function output_bgrams = decode_file(obj, filename, Ndecimation = 1)

            % open bgram file at the beginning
            fid = fopen(filename, 'r');

            % find out file size in bytes
            fseek(fid, 0, SEEK_END);      % this line jumps to the end of the file
            file_size_bytes = ftell(fid); % this line tells us how far through reading the file we are
            fseek(fid, 0);                % this line jumps back to the beginning of the file so we can read in all the agrams
            ...
            printf("Decoding %s of size %d bytes; bgram size: %d bytes\n", filename, file_size_bytes, BgramSizeBytes);
            ...
            do
                ...
                if parse_started == 1 && mod(numel(output_bgrams), NumberOfBeams) == 0 
                  ...
                  fseek(fid, Ndecimation * NumberOfBeams * BgramSizeBytes, SEEK_CUR);
                ...
                if parse_started == 0 && bgram.sCommon.u16_BeamIndex ~= 0 % not start condition
                  ...
                  printf ('Ping Index %d; Beam Index %d ignored at start of file\n', bgram.sCommon.u32_PingCounter, bgram.sCommon.u16_BeamIndex);
                  ...
                  k = k + 1;
                   ...
                   %if mod(bgram.sCommon.u32_PingCounter, Ndecimation) == 0
                     ...
                     if bgram.sCommon.u16_BeamIndex ~= mod(output_bgrams(k - 1).sCommon.u16_BeamIndex + 1, bgram.sCommon.u16_NoBeams) % are we on the next beam?
                      ...
                      np -= (output_bgrams(k - 1).sCommon.u16_BeamIndex + 1);
                      np -= (bgram.sCommon.u16_NoBeams - bgram.sCommon.u16_BeamIndex);
            ...
            printf("output_bgrams size: %d\n", numel(output_bgrams));   
            
            if isinteger(numel(output_bgrams) / NumberOfBeams) == false
              
              printf("Additional unwanted bgrams\n")
        ...
        function data_arrays = ensembles_to_data_arrays(obj, ensembles)
            ...
            printf("--- BGRAM ensembles_to_data_arrays ---\n")
